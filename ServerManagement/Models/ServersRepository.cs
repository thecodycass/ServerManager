...
public class ServersRepository
{
    private static List<Server> servers = new List<Server>()
    {
        ...
        new Server { ServerId = 11, Name = "Server11", City = "Halifax"},
    ...
    public static List<Server> GetServersByCity(string city) => servers.Where(s => s.City == city).ToList();
    ...
    public static void UpdateServer(int serverId, Server server)
    {
        ...
        var serverToUpdate = servers.FirstOrDefault(s => s.ServerId == server.ServerId);
    ...
    {
        return servers.Where(s => s.Name.Contains(serverFilter.ToLower(), StringComparison.OrdinalIgnoreCase)).ToList();
    }
}